...
Pacchetto di ripristino `.zip` file
 a| 
È necessario link:downloading-recovery-package.html["Scarica il pacchetto di ripristino più recente"] `.zip` file (`sgws-recovery-package-_id-revision_.zip`). È possibile utilizzare il file Recovery Package per ripristinare il sistema in caso di errore.
